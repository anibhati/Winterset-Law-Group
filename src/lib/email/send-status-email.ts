...
const STATUS_COPY: Record<StatusType, { subject: string; headline: string; body: string }> = {
  APPROVED: {
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border-radius:8px; overflow:hidden;">
        ...
        <tr><td style="padding:40px;">
          <p style="margin:0 0 8px; color:#6b7280; font-size:13px; text-transform:uppercase; letter-spacing:0.05em;">${requestType.toUpperCase()}</p>
          <h2 style="margin:0 0 24px; color:#10283B; font-size:20px; font-weight:700;">${copy.headline}</h2>
          <p style="margin:0 0 16px; color:#1a1a1a; font-size:16px; line-height:1.5;">Hi ${name},</p>
          <p style="margin:0 0 24px; color:#1a1a1a; font-size:16px; line-height:1.5;">${copy.body}</p>
          ${staffNotes ? `
          <div style="background-color:#f9fafb; border-left:4px solid #B1784D; border-radius:4px; padding:16px; margin:0 0 24px;">
            <p style="margin:0 0 6px; color:#B1784D; font-size:13px; font-weight:700; text-transform:uppercase;">Note from our team</p>
            <p style="margin:0; color:#1a1a1a; font-size:15px; line-height:1.5;">${staffNotes}</p>
          </div>` : ""}
          <table cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
            <tr><td style="background-color:#B1784D; border-radius:6px; text-align:center;">
              <a href="${dashboardUrl}" style="display:inline-block; padding:14px 32px; color:#ffffff; font-size:16px; font-weight:600; text-decoration:none;">View My Account</a>
            </td></tr>
          </table>
          <p style="margin:0 0 24px; color:#6b7280; font-size:14px;">Questions? Call us at <a href="tel:${FIRM_PHONE}" style="color:#10283B; font-weight:600;">${FIRM_PHONE}</a>.</p>
          <hr style="border:none; border-top:1px solid #e5e7eb; margin:0 0 24px;" />
          <p style="margin:0; color:#1a1a1a; font-size:14px; line-height:1.6;">Christopher J. Stevens<br /><span style="color:#6b7280;">Winterset Law Group</span></p>
        </td></tr>
        <tr><td style="background-color:#f9fafb; padding:24px 40px; text-align:center;">
          <p style="margin:0; color:#9ca3af; font-size:12px;">This is an automated message from Winterset Law Group. Please do not reply directly to this email.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
